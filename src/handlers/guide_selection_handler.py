...
from src.guides.viz_posts import VIZ_SECOND_POST_NUMBER, VIZ_THIRD_POST_NUMBER
...
class GuideSelectionHandler:
    ...
    def register_in_dispatcher(self, dispatcher: Dispatcher) -> None:
        ...
        dispatcher.callback_query.register(self.__send_viz_third_post, F.data == GuideCallbackData.VIZ_NEXT_AFTER_SECOND)
        dispatcher.callback_query.register(self.__handle_viz_next, F.data == GuideCallbackData.VIZ_NEXT_AFTER_THIRD)
    ...
    async def __send_viz_third_post(self, callback: CallbackQuery) -> None:
        await callback.answer()
        if callback.message is None:
            return
        await self.__post_sender.send_post(
            callback.message,
            self.__posts.get_post(VIZ_THIRD_POST_NUMBER),
            self.__keyboards.build_viz_next_keyboard(GuideCallbackData.VIZ_NEXT_AFTER_THIRD),
        )
