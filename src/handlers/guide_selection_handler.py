...
class GuideSelectionHandler(VizRouteHandlerMixin, VizLaterRouteHandlerMixin, VizFinalRouteHandlerMixin, VizNewestRouteHandlerMixin, VizLatestRouteHandlerMixin, VizFeedbackFinishHandlerMixin):
    ...
    def register_in_dispatcher(self, dispatcher: Dispatcher) -> None:
        dispatcher.callback_query.register(
            self.__select_big_konny, F.data == GuideCallbackData.SELECT_BIG_KONNY
        )
        dispatcher.callback_query.register(
            self.__send_viz_second_post, F.data == GuideCallbackData.VIZ_NEXT
        )
