...
class VizNewestRouteHandlerMixin:
    async def _send_viz_eighteenth_post(self, callback: CallbackQuery) -> None:
        await self._send_viz_post(
            callback, VIZ_EIGHTEENTH_POST_NUMBER, GuideCallbackData.VIZ_NEXT_AFTER_EIGHTEENTH
        )

    async def _send_viz_nineteenth_post(self, callback: CallbackQuery) -> None:
        await self._send_viz_post(
            callback, VIZ_NINETEENTH_POST_NUMBER, GuideCallbackData.VIZ_NEXT_AFTER_NINETEENTH
        )
    ...
    async def _send_viz_twenty_first_post(self, callback: CallbackQuery) -> None:
        await self._send_viz_post(
            callback,
            VIZ_TWENTY_FIRST_POST_NUMBER,
            GuideCallbackData.VIZ_NEXT_AFTER_TWENTY_FIRST,
        )
