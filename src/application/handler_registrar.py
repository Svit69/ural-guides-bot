...
class HandlerRegistrar:
    ...
    def register_handlers(self, dispatcher: Dispatcher) -> None:
        ...
        city_access_repository = CityAccessRepository(self.__connections)
        post_provider = PostProvider(post_repository, media_repository)
        StartCommandHandler(
            post_repository, media_repository, user_repository, viz_access_repository,
            city_access_repository, self.__settings.viz_guide_price_rub, self.__settings.city_guide_price_rub
        ).register_in_dispatcher(dispatcher)
        GuideSelectionHandler(post_provider).register_in_dispatcher(dispatcher)
        PaymentHandlerRegistrar(self.__settings, self.__connections).register(dispatcher, post_provider, admin_repository)
        SubscriptionCheckHandler(ChannelSubscriptionChecker(self.__settings.subscription_channel_username), post_provider).register_in_dispatcher(dispatcher)
