...
class CityPaymentRepository:
    ...
    def get_payment(self, user_id: int) -> dict[str, object] | None:
        with self.__connections.open_connection() as connection:
            row = connection.execute(
                "select * from city_payments where user_id = ?", (user_id,)
            ).fetchone()
    ...
    def save_payment(self, user_id: int, payment: dict[str, str]) -> None:
        with self.__connections.open_connection() as connection:
            connection.execute(
                """
                insert into city_payments values (?, ?, ?, ?, ?)
                ...
                """,
                (user_id, payment["payment_id"], payment["status"],
                 payment.get("confirmation_url"), datetime.now(timezone.utc).isoformat()),
            )

    def update_status(self, user_id: int, status: str) -> None:
        with self.__connections.open_connection() as connection:
            connection.execute(
                "update city_payments set status = ?, updated_at = ? where user_id = ?",
