...
def test_builds_admin_guide_selection_keyboard() -> None:
    ...
    assert keyboard.inline_keyboard[1][0].callback_data == "admin:guide:city_walk"
    assert keyboard.inline_keyboard[2][0].text == "Большой Конный п-ов"
...
def test_builds_admin_main_keyboard_with_viz_buyers_button() -> None:
    ...
    assert keyboard.inline_keyboard[4][0].callback_data == AdminCallbackData.CLEAR_FEEDBACK
...
def test_parses_admin_selection_callbacks() -> None:
    ...
    assert AdminCallbackData.parse_guide_id("admin:guide:city_walk") == "city_walk"
